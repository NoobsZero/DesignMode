...
def init_es():
    return Elasticsearch(["192.168.90.10:9200"])
...
def query_data(log_date, puid, count):
    ...
    data_array = es.search(index=index_name, q='offset: "' + str(puid) + '"', doc_type='doc', params=para,
                           size=int(count))
...
def query_data_by_page(log_date, puid, page_count, page_num):
    ...
    data_array = es.search(index=index_name, q='offset: "' + str(puid) + '"', doc_type='doc', params=para,
                           size=int(page_count), from_=from_page)
...
def main(argv):
    ...
    for opt, value in opts:
        ...
        if opt in ('-h', '--help'):
            print("")
            print(
                "Usage：python query_client_log.py -p puid [-c count -l log_date -i [-o page_count -n page_count]] | --puid=puid ....")
            print("-p, --puid           用户id")
