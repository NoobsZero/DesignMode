...
def get_url_lis(browser, name='六盘水市', url='https://poi.mapbar.com/liupanshui/'):
    # cityCrossing = []
    for i in zm:
        index = 0
        while index < 3:
            try:
                print(os.path.join(url, f'GA0{i}/'))
                browser.get(os.path.join(url, f'GA0{i}/'))
                wait = WebDriverWait(browser, 10)
                wait.until(EC.presence_of_element_located((By.XPATH, '/html/body/div[2]/div[4]')))
            except TimeoutException as e:
                index += 1
                print(index)
            break
        only_a_tags = SoupStrainer('div', attrs={"class": "whiteBor layout_a", "style": "margin-left:8px;"})
        p_x_list = BeautifulSoup(browser.page_source, 'html.parser', parse_only=only_a_tags)
        data = [{'city_name': name, 'type': 'address', 'address': a.text, 'hash': a['href']} for a in
                p_x_list.find_all('a')]
        for dict_json in data:
            print(dict_json)
            with open('csCrossing.txt', 'a') as f:
                f.write(json.dumps(dict_json))
                f.write('\n')
        # with open(filename, 'a') as file_object:
        # cityCrossing += [{'city_name': name, 'type': 'address', 'address': a.text, 'hash': a['href']} for a in
        #                  p_x_list.find_all('a')]
    # return cityCrossing
...
if __name__ == '__main__':
    browser = getBrowser()
    cs = ["广州市", "哈尔滨市", "哈密市", "胡杨河市", "香港特别行政区", "惠州市", "淮安市", "杭州市", "合肥市",
    ...
    for i in city:
        ...
        if name in cs:
            print(name)
            get_url_lis(browser=browser, name=name, url=f'https://poi.mapbar.com/{ename}/')
            # for s in get_url_lis(browser=browser, name=name, url=f'https://poi.mapbar.com/{ename}/'):
            #     print(s)
            # requests.post(url='http://192.168.50.75:5001/map/city', data=s)
        time.sleep(2)
    # cityCrossing = []
    ...
    #     name = i['name']
    #     if name in cs:
    #         url = f'https://poi.mapbar.com/{ename}/GA0/'
