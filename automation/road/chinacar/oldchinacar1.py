...
def getProductioninformation(soup):
    ...
    Productioninformation['field_2'] = Vehicletype.replace('法律法规', '').strip()
    ...
    try:
        ...
        Accordingtothestandard = soup.select_one(
            '#con_two_p1 > table > tbody > tr:nth-child(40) > td:nth-child(4) > span').text
    ...
    try:
        ...
        Identifytheenterprise = soup.select_one('#con_two_p1 > table > tbody > tr:nth-child(45) > td:nth-child(2)').text
    ...
    try:
        ...
        Logotrademark = soup.select_one('#con_two_p1 > table > tbody > tr:nth-child(45) > td:nth-child(4)').text
    ...
    try:
        ...
        Identificationmodel = soup.select_one('#con_two_p1 > table > tbody > tr:nth-child(46) > td:nth-child(2)').text
...
def getjson(file):
    ...
    for i in matches:
        if 'img' in i:
            urls = re.sub('clcppic', "spic", re.findall(r'(https?://\S*?jpg|\S*?JPG)+', str(i))[1])
        else:
            result = re.sub('\W+', '', i.split(':')[1]).replace("_", '')
            urlList[urls] = ['http://chinacar.com.cn/ggcx_new/search_view.html?id=' + result, result]
    return urlList

def registerUrl(oldDic):
    try:
        ...
        newDic = {}
        for dic in oldDic:
            if oldDic[dic][1] not in data:
                newDic[dic] = oldDic[dic]
        return newDic
    except Exception as e:
        print(e)

if __name__ == '__main__':
    fo = open('file.txt', encoding='utf8', errors='ignore')
    fileList = fo.readlines()
    matches = (x for x in fileList if ('tarid' in x and 'tarid_nobase' not in x) or ('img' in x))
    urlList = {}
    url = None
    ...
    urlList = registerUrl(urlList) # 过滤重复数据
    for key in urlList:
        try:
            ...
            try:
                ...
                for i in engineparameters:
                    print(i)
                    enJson = json.dumps(engineparameters, ensure_ascii=False).encode('utf-8')
                    requests.post(url=url2, data=enJson)
            except IndexError as e:
                print('下标越界,没有此字段信息:', urlList[key][1])
            lis_str.clear()
        except Exception as e:
            fo = open("error.txt", "a")
            fo.write("图片地址:" + str(key) + ",页面地址:" + str(urlList[key][0]) + ",id:" + urlList[key][
