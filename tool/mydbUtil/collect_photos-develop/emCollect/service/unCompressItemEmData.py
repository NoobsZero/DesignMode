...
class UnCompressItemWebExportWuPan(UnCompressItemWebExport):
    ...
    def fixDBName(self):
        # todo 查询最新的 number
        # 否则 00 ;
        self.fixNumber = getNextWuPanNumber(self.ormSess, self.baseCityInfo.cityCode, self.baseCityInfo.dateStr,
                                            self.packageName)
    ...
    def genarateOrmRecordObj(self):
        ...
        if self.callBackGenerateRecord is not None:
            val = self.callBackGenerateRecord(self.baseCityInfo.cityCode, self.baseCityInfo.deviceType,
                                              ...
                                              self.confSaveRules.getRootAlgConfPath(), os.path.basename(self.filePath),
                                              self.fixNumber)
