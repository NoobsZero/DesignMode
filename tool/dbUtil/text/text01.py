    ...
    if len(info_datas_lis) != 0:
        for info in info_datas_lis:
            ...
            info['category_id'] = \
                literal_eval(
                    requests.get(
                        'http://192.168.50.100:3018/api/v1/chejian/get_id?field_name=code&category={}&reason={}'
                            .format(info.pop('category', None), info.pop('reason', None))).text)['id']
            cs_id = \
                literal_eval(
