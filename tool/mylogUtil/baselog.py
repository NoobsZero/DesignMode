...
logging.basicConfig(level=logging.INFO, format='[%(asctime)s_%(name)-8s-%(levelname)-8s]:%(message)s')
# 创建
...
def registerLoggerHandle(fileName):
    ...
    formatter = logging.Formatter('[%(asctime)s_%(name)-8s-%(levelname)-8s]:%(message)s')
